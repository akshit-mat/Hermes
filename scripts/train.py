...
from hermes.training.trainer import create_local_logger, resolve_device, set_deterministic_seed, train_basic


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the basic Hermes training loop.")
    ...
    seed = int(training_config.get("seed", config.get("data", {}).get("seed", 42)))
    set_deterministic_seed(seed)
    tokenizer = HermesTokenizer.load(config["tokenizer"]["output_path"])
    pad_token_id = tokenizer.token_to_id("<pad>")
    if pad_token_id is None:
        raise ValueError("Loaded tokenizer does not define <pad>")
    model = build_model_from_config(config, pad_token_id=pad_token_id)
    logger = create_local_logger(training_config.get("log_path"))
    logger(f"seed={seed}")
    split_dir = Path(tokenized_config["output_dir"])
    ...
    history = train_basic(
        ...
        max_steps=int(args.max_steps or training_config["max_steps"]),
        ...
        wandb_project=str(config.get("logging", {}).get("project", "hermes")),
        wandb_run_name=config.get("logging", {}).get("run_name"),
        wandb_config=config,
    )
    print(f"completed_steps={len(history['train_loss'])}")
